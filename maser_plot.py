...
class maser_plot:
    def __init__(self):
        ...
        parser.add_argument(
            ...
            type=bool,
            default=False #Velocity standardised by default
        )
        ...
        self.standardise_velocity = bool(args.standardise_velocity)
    ...
    def write_plot(self, velocity_array, spectra_array_on, spectra_array_off, fourier_counts, fourier_velocity, intensity_scale, plot_name, vel_source, vel_source_low, vel_source_high, epoch, format, inclusion_size, polynomial_order):
        #Write out the file to a .spt file, to be used with Simon's script 'splot.pl'

        #Different plotting options are done seperately           

        #TODO: FOURIER IN OPTION 1, also <30 if statement clean  up
            ...
            for i in range(len(self.file_name_list)):
                ...
                if scan_tag != 'off':
                    ...
                    if self.standardise_velocity == True:
                        ...
                        #This way the bins will be at the same location every observation
                        ...
                        #TEST
                        #correction_remainder = velocity_correction / delta_vel

                        integer_counts = np.arange(0, Nfft, 1) + correction_remainder
                        ...
                        centre_maser_offset_bins = (centre_freq - maser_freq) / df
                    ...
                    RMS, baseline_coefficients = self.write_plot(velocity_array = corrected_velocity_array, spectra_array_on = spectra_array, spectra_array_off = off_spectra, fourier_counts=integer_counts, fourier_velocity=shifted_velocity, intensity_scale = self.intensity_scale, plot_name = plot_name, vel_source = vel_centre, vel_source_low = vel_low, vel_source_high = vel_high, epoch = epoch_numpy, format = self.format, inclusion_size = self.velocity_inclusion_size, polynomial_order = self.baseline_polynomial_order)
