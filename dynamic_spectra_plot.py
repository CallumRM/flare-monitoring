...
#TODO: Add options to customise things like axis labels and title, currently a bit scuffed.
class dynamic_spectra:
    ...
    def plot_spectra(self):
        #Set some graphics options for plotly
        ...
        figure = go.Figure(data = go.Heatmap(
                    z = self.intensities,
                    x = velocity -0.07,#HACK TO FIX VELOCITY FOR THE PLOT CREATED FOR MY THESIS. IN FUTURE, ADD AS REAL OPTION
                    y = self.epochs,
                    colorbar = dict(title='Normalised<br>Intensity'),
                    ygap=0.5,
                    colorscale='Jet'
                    ))
        figure.update_layout(
                yaxis_title="Epoch (Day of 2021)",
                xaxis_title="Velocity w.r.t LSRK (kms<sup>-1</sup>)",
                yaxis=dict(tickformat="%j"),
